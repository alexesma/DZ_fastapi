...
from dz_fastapi.services.autopart_dedup import canonicalize_autoparts_by_brand_synonyms
...
async def test_canonicalize_autoparts_by_brand_synonyms_merges_duplicates(
    ...
    customer_order_items = (
        (
            await test_session.execute(
                select(CustomerOrderItem).where(CustomerOrderItem.order_id == customer_order.id)
            )
        )
        .scalars()
        .all()
