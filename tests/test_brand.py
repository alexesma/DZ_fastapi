...
from dz_fastapi.models.partner import PriceList, PriceListMissingBrand, ProviderPriceListConfig
...
async def test_upload_logo_file_size_exceeds(test_session, created_brand: Brand):
    ...
    img_byte_arr = BytesIO()
    # Отключаем оптимизацию для увеличения размера
    ...
    # Проверки
    assert response.status_code == 400, (
        f"Expected status code 400, but got {response.status_code}. " f"Response: {response.text}"
    )
