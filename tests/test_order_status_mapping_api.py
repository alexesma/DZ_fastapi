...
async def test_list_unmapped_statuses_returns_only_unresolved(
    ...
    response = await async_client.get("/admin/order-status-mappings/unmapped")
...
@pytest.mark.asyncio
async def test_create_supplier_email_mapping_with_action_only(
    async_client,
    test_session,
):
    admin = await _create_admin(test_session)
    ...
    assert response.status_code == 200, response.text
    payload = response.json()
    assert payload["source_key"] == EXTERNAL_STATUS_SOURCE_SUPPLIER_EMAIL
    assert payload["supplier_response_action"] == "FULL_CONFIRM"
    assert payload["internal_order_status"] is None
    assert payload["internal_item_status"] is None
