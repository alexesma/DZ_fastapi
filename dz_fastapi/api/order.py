...
from dz_fastapi.schemas.order import (
    ...
    CustomerOrderPeriodReportResponse,
    OrderItemOut,
    OrderOut,
    OrderPositionOut,
...
async def _resolve_site_provider_id(
    ...
    if name_cache_key:
        ...
        # Привязка, заведённая человеком при объединении дубля, важнее
        # совпадения имён — поэтому идёт первой.
        ...
        if reference is not None and reference.is_active:
            provider_cache[name_cache_key] = reference.provider_id
            if id_cache_key:
                provider_cache[id_cache_key] = reference.provider_id
            return reference.provider_id

        # Без учёта регистра: сайт пишет «Cosmopart», в каталоге
        # «COSMOPART», и точное сравнение плодило дубли.
...
@router.get(
    "/autopurchase-top/customer-order-period-report/preview",
    response_model=CustomerOrderPeriodReportResponse,
    summary="Предпросмотр отчёта по заказам клиентов за два периода",
)
async def preview_customer_order_period_report_view(
    ...
    try:
        return await build_customer_order_period_report_data(
            session=session,
