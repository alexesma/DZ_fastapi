...
async def get_autopart_offers(
    ...
    partial: bool = Query(False, description="Искать по части OEM номера"),
...
async def create_storages_bulk(
    ...
    return [_storage_to_response(location) for location in reloaded_locations]
...
async def list_all_applicability_nodes(
    ...
    result = await session.execute(
        select(ApplicabilityNode).order_by(
