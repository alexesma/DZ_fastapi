...
async def change_brand_name(brand_name: str) -> str:
    ...
    brand_name = ''.join(
        [char.upper() if char in string.ascii_letters
         else char for char in brand_name]
    )
...
async def change_storage_name(storage_name: str) -> str:
    storage_name = ''.join(
        [char.upper() if char in string.ascii_letters
         else char for char in storage_name]
    )
...
async def apply_filters(autopart_associations, config):
    ...
    if min_price is not None:
        autopart_associations = [
            assoc for assoc in autopart_associations if float(
                assoc.price
            ) >= min_price
        ]
...
async def calculate_price(assoc, config):
    ...
    individual_markup = config.individual_markups.get(
        str(supplier_pricelist_id)
    )
