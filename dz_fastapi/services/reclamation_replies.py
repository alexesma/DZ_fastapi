...
def build_customer_reply_template(
    ...
    if kind == "approved":
        body = (
            f"Здравствуйте!\n\n"
            f"По вашей рекламации {doc} принято решение согласовать возврат"
            f" следующих позиций:\n{items}\n\n"
            f"{('Комментарий: ' + reason + chr(10) + chr(10)) if reason else ''}"
            f"Пожалуйста, оформите возврат согласно нашим правилам. "
            f"По вопросам отгрузки возврата свяжемся дополнительно.\n\n"
            f"С уважением,\nотдел рекламаций"
        )
    elif kind == "rejected":
        body = (
