...
from dz_fastapi.core.constants import IMAP_FETCH_PER_ACCOUNT_TIMEOUT, IMAP_SERVER
from dz_fastapi.core.email_folders import DEFAULT_IMAP_FOLDER, parse_imap_additional_folders
...
def _fetch_inbox_message_by_uid_imap_sync(
    *,
    ...
    """
    try:
        with _create_mailbox(host, port, True).login(
            email, password
        ) as mailbox:
...
async def restore_inbox_email_attachments_from_source(
    ...
    *,
    ...
    account = await crud_email_account.get(
        session, inbox_email.email_account_id
    )
    if account is None:
        return False
...
async def _restore_existing_email_attachments_if_missing(
    ...
    *,
    ...
    has_missing_files = not existing_info or any(
...
async def _find_matching_customer_order_configs(
    ...
    *,
    ...
    for config in configs:
        if email_account_id is not None and config.email_account_id not in (
            None,
            email_account_id,
        ):
            continue
        ...
        if config.order_emails:
            extra = (
                config.order_emails
                if isinstance(config.order_emails, list)
                else []
            )
    ...
    *,
    ...
    try:
        if rule_type == "customer_order":
            from dz_fastapi.services.customer_orders import process_customer_orders
            ...
            if not matched_config_ids:
                ...
                if failed_configs and triggered_config_ids:
                    ...
                    processing_result["reason"] = "Обработка успешно запущена"
            ...
            matched_config_ids = [int(config.id) for config in matched_configs]
            processing_result["matched_config_ids"] = matched_config_ids
            processing_result["summary"]["matched_configs_count"] = len(
                matched_config_ids
            )
    ...
    for config in configs:
        ...
        if filepath:
            try:
                ...
                logger.error(
                    "Ошибка обработки прайса provider=%s: %s", provider.id, e
                )
    ...
    try:
        ...
        await create_admin_notifications(
            ...
            message=(
                ...
                f"Тема: {inbox_email.subject}" + tracking_info
            ),
            level="info",
        )
    except Exception as e:
    ...
    if (
        ...
        try:
            ...
            if provider:
                ...
                sender_email = (
                    str(inbox_email.from_email or "").strip().lower()
                )
                previous_sender = (
                    str(provider.email_incoming_price or "").strip().lower()
                )
                ...
                if rule_type == "price_list" and cfg_mode != "skip":
                    ...
                    if cfg_mode == "existing" and provider_config.config_id:
                        ...
                        new_pl = ProviderPriceListConfigCreate(
                            ...
                            filename_pattern=(price_filename_pattern or None),
                ...
                # --- order_reply / document: SupplierResponseConfig ---
                    ...
                    response_type = (
                        str(
                            getattr(response_type, "value", response_type)
                            or ""
                        )
                        .strip()
                        .lower()
                    ...
                    if cfg_mode == "existing" and provider_config.config_id:
                        sr_cfg = await crud_supplier_response_config.get_by_id(
                            config_id=provider_config.config_id,
                            session=session,
                        )
                        if sr_cfg:
                            ...
                            sender_emails = list(
                                getattr(sr_cfg, "sender_emails", None) or []
                            )
                            if (
                                inbox_email.from_email
                                and inbox_email.from_email not in sender_emails
                            ):
    ...
    # ------------------------------------------------------------------
        ...
        try:
            ...
            if config_mode == "new":
                ...
                if existing_cfg is None and (selected_config_id is None):
                    existing_cfg = (
                        ...
                        for field in col_fields:
                            if order_config.get(field) is not None:
                                if field == "order_start_row":
                                    ...
                                    update_data[field] = _to_zero_based_column(
                                        order_config[field]
                                    )

                    if update_data:
                        await crud_customer_order_config.update(
                            session, existing_cfg, update_data
                        )
                        action = "updated"
                        note = (
                ...
                configs_set.append(
                    {
                        ...
                        "entity_name": ("Supplier response registry"),
                        "action": "queued",
                        "note": (
...
async def cleanup_inbox_emails(
    ...
    (
        ...
        orphan_dirs_deleted,
    ) = await _cleanup_orphan_inbox_attachment_files(
        session=session,
        cutoff=cutoff,
    )
    logger.info(
