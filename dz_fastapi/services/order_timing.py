...
async def compute_customer_order_windows(
    ...
    entries_by_group: dict[tuple[int, int], list[tuple[float, date]]] = (
        defaultdict(list)
    )
    for r in rows:
    ...
    for (customer_id, weekday), entries in entries_by_group.items():
        ...
        if split_m is not None:
            ...
            w0 = _build_window(
                customer_id, name, weekday, morning_entries, 0, split_m
            )
            w1 = _build_window(
                customer_id, name, weekday, evening_entries, 1, split_m
            )
...
async def _count_orders_in_window(
    ...
    *,
    ...
    stmt = select(func.count(CustomerOrder.id)).where(
...
async def is_in_any_order_window(session: AsyncSession) -> bool:
    ...
    for w in windows:
        ...
        if w.window_start <= current_t <= w.window_end:
            return True
        window_end_dt = datetime.combine(today, w.window_end).replace(
            tzinfo=tz
        )
        ...
        if elapsed > GRACE_PERIOD_SECONDS:
            continue
        window_start_dt = datetime.combine(today, w.window_start).replace(
            tzinfo=tz
        )
    ...
    for w in today_windows:
        ...
        if w.split_minute is not None:
            ...
            if w.window_index == 0:
                window_orders = [
                    (oid, rat)
                    for oid, rat in all_customer_orders
